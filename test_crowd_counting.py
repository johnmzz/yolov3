...
def detect_and_count(save_txt=False, save_img=False, ROI="vertical"):
    ...
    count_dict = {}
    if opt.count_file:
        with open(opt.count_file) as count_file:
    ...
    for path, img, im0s, vid_cap in dataset:
        img_name = path.split('/')[-1]          # for CC evaluation
        t = time.time()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            for track in tracks_active:
                ...
                if track['direction'] == "U2D" and track['centroid'][1] > H//2 and track['counted'] == False:
                    total_U2D += 1
                    track['counted'] = True

                if track['direction'] == "D2U" and track['centroid'][1] < H//2 and track['counted'] == False:
                    total_D2U += 1
                    track['counted'] = True
            ...
            # Evaluate MAE
            detection_nums[img_name] = len(tracks_active)
            ...
            if ROI == 'horizontal':
                cv2.line(im0, (0, H // 2), (W, H // 2), (0, 0, 255), 2)
            elif ROI == 'vertical':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--count_file', type=str, help='count file path')
